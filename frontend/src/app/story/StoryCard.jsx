...
const StoryCard = ({ isAddStory, story }) => {
  ...
  const userPlaceholder = story?.user?.name
    ?.split(" ")
    .map((name) => name[0])
    .join("");
    ...
    <>
        ...
        <CardContent className="p-0 h-full">
          {isAddStory ? (
            <div className="w-full h-full flex flex-col">
              ...
              <div className="h-1/4 w-full bg-white dark:bg-gray-800 flex flex-col items-center justify-center">
                <Button
                  variant="ghost"
                  size="sm"
                  className="p-0 h-8 w-8 rounded-full bg-green-500 hover:bg-green-600 "
            ...
            <>
              ...
              <div className="absolute top-2 left-2 ring-2 ring-green-600 rounded-full ">
     ...
     {showPreview && (
      <ShowStoryPreview
        ...
        username = {isNewStory ? user?.name : story?.user?.name}
